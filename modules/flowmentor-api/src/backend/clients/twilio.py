...
class TwilioClient:
    ...
    async def init_connection(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Twilio connection established for account: {account.friendly_name}"
            )
    ...
    async def send_sms(self, to_phone_number: str, message: str) -> dict:
        ...
        try:
            message_obj = self.client.messages.create(
                body=message, from_=self.config.from_phone_number, to=to_phone_number
            )
            ...
            logger.info(
                f"SMS sent successfully to {to_phone_number}, SID: {message_obj.sid}"
            )
            return result
