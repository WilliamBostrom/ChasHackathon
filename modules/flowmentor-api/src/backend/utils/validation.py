...
def check_python_compatibility() -> List[str]:
    ...
    if sys.version_info < (3, 11):
        issues.append(
            f"Python {sys.version_info.major}.{sys.version_info.minor} detected. Minimum Python 3.11 required."
        )
...
def check_uuid_usage() -> List[str]:
    ...
    try:
        ...
        if hasattr(UUID, "_from_int"):
            issues.append(
                "WARNING: UUID._from_int() detected - use UUID(int=...) instead"
            )
                ...
                if in_enum and "=" in stripped:
                    ...
                    if len(parts) == 2:
                        ...
                        if value != value.lower():
                            issues.append(
                                f"Enum {enum_name}: '{value}' should be lowercase for database compatibility"
                            )
        ...
        if workflows_path.exists():
            for py_file in workflows_path.glob("*.py"):
                ...
                if "asyncio.sleep" in content and "@workflow.defn" in content:
                    issues.append(
                        f"{py_file.name}: Use workflow.sleep() not asyncio.sleep() in workflows"
                    )
                ...
                if "wait_condition" in content:
                    ...
                    for i, line in enumerate(lines):
                        if "wait_condition" in line and "lambda:" in line:
                            ...
                            if (
                                "if not" in next_text
                                and "wait_condition" not in next_text
                            ):
                                issues.append(
                                    f"{py_file.name}:{i + 1}: Possible wait_condition logic error - check boolean handling"
                                )
...
def print_validation_report():
    ...
    if total_issues == 0:
        ...
        print(
            f"⚠️  Found {total_issues} potential issues. Review and fix before deployment."
        )
